...
- **GAS_ENABLE**: in ACS non entra subito. Il firmware misura da quanto tempo la PDC sta lavorando/chiedendo aiuto e abilita il gas solo a finestre progressive (`0-30 min` sempre OFF, `30-60 min` solo se il boiler PDC e' molto lontano dal target, oltre `60 min` con soglie piu permissive). Se il boiler solare puo' ancora aiutare, aggiunge un ulteriore ritardo prima di accendere il gas. Restano poi validi il boost dopo lavoro continuo C2 su piscina e il caso piscina appena riempita (placeholder).
...
- **CR (ricircolo collettore)**: `control_recirc.py`
  ...
  - In antilegionella `S6/S7` non partecipano al criterio di completamento: il ciclo porta prima il boiler PDC alla soglia di avvio (`S4 >= target antilegionella`, `S5 >= 68 C`), poi attiva `CR`.
  - Durante la fase con `CR` attivo, il boiler PDC deve restare sopra la stessa soglia (`S4 >= target antilegionella`, `S5 >= 68 C`) per 1800 secondi totali; se la temperatura scende, il timer viene messo in pausa e non azzerato.
